...
class Plotter(object):
    ...
    def __init__(self, title='title'):
        ...
        self.lines = [[]] * 3   # 8 line plots and 6 axes
...
class MagCycle(object):
    ...
    _Lakeshore340_GPIB_ADDR = 1
    _Lakeshore332_GPIB_ADDR = 2
    ...
    def __init__(self
        ...
        self.timestep_magup = timestep_magup # How fast to ramp up voltage in sec.
        self.timestep_magdown = timestep_magdown
        self.initial_setvoltage = 0
        self.inductive_voltage_limit = inductive_voltage_limit # in V

        # plot initialization
        self.plotter = Plotter(title=f'ADR2 MAG CYCLE MONITORING @{datetime.datetime.now():%x,%X}')
    ...
    def configure_instrument(self):
        ...
        self.Lakeshore50K_3K = Lakeshore340.Lakeshore340(MagCycle._Lakeshore340_GPIB_ADDR) # 50K, 3K
        self.LakeshoreGGG_FFA = Lakeshore332.Lakeshore332(MagCycle._Lakeshore332_GPIB_ADDR)  # GGG, FFA
        ...
        if self.power_supply.getOutputStatus(): # if output is on, read set voltage and set current=max_current
            ...
            self.power_supply.setCurrent(self.max_current)  # for voltage-biased mode
    ...
    def mag_up(self):
        ...
        """
        # Check if 3K is cold
        T_3K = self.Lakeshore50K_3K.getTemperature('A')
        if T_3K > 4.5:
        ...
        for volt in self._magup_voltages():
            ...
            if heat_switch_status == 'OPEN' and self.LakeshoreGGG_FFA.getTemperature('A') > 3:
    ...
    def run_mag_cycle(cls, **kwargs):
        ...
        logging.info(f'Mag cycle is completed @{datetime.datetime.now():%X}! Yeah!')
    ...
    def run_mag_down_only(cls, **kwargs):
        ...
        logging.info(f'Magdown is completed @{datetime.datetime.now():%x, %X}! Yeah!')
