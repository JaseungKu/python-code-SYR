...
class CPW(object):
    ...
    def epsilon_eff(self):
        ...
        epsilon_eff = 1 + (self.epsilon_r - 1)/2*ellipk(k2)*ellipk(k1_p)/ellipk(k2_p)/ellipk(k1) 
    ...
    def C_l(self):
        ...
        Goppl paper uses the same expression, C_l = 4 * epsilon_0 * self.epsilon_eff * ellipk(k1)/ellipk(k1_p)
        """
        k1 = self.w / self.b
        k1_p = np.sqrt(1 - k1**2)
        k2 = np.sinh(np.pi*self.w/4/self.h) / np.sinh(np.pi*self.b/4/self.h)
        ...
        C1 = 2 * epsilon_0 * ellipk(k1)/ellipk(k1_p)
        C2 = 2 * epsilon_0 * (self.epsilon_r - 1) * ellipk(k2)/ellipk(k2_p)
...
#         Z0 = 60 * np.pi / np.sqrt(self.epsilon_eff) / (ellipk(k)/ellipk(k_p) + ellipk(k3)/ellipk(k3_p))
...
#         return Z0
    ...
    def Z0(self):
        ...
        Z0 = 30 * np.pi/np.sqrt(self.epsilon_eff) * ellipk(k_p)/ellipk(k)
...
class QuarterWaveResonator(CPW):
    ...
    def resonator_length(self):
        ...
        Lambda*freq = c/sqrt(epsilon_eff)=1/sqrt(L_l*C_l)
        """
        ...
        l = 1/np.sqrt(total_induct_per_length * self.C_l) / (w_r_pulled/2/np.pi) / 4
        
        return l
    
    def get_dressed_res_freq(self):
        w_r_pulled = self.w_r  # TODO: calculate frequency shift by coupling to feedline
        return w_r_pulled
